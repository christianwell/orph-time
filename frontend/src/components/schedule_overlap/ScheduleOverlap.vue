...
export default {
  ...
  props: {
    ...
    weekOffset: { type: Number, default: 0 }, // Week offset used for displaying calendar events on weekly Orph-times
  ...
  computed: {
    ...
    /** Parses the responses to the Orph-time, makes necessary changes based on the type of event, and returns it */
    parsedResponses() {
      const parsed = {}
    ...
    confirmScheduleEvent(googleCalendar = true) {
      ...
      if (googleCalendar) {
        ...
        url = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(
          ...
          "\n\nThis event was scheduled with Orph-time: https://timeful.app/e/"
